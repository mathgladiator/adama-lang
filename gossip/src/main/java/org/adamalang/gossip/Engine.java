...
public class Engine implements AutoCloseable {
  ...
  public void setWatcher(Consumer<Collection<Endpoint>> watcher) {
    executor.execute(new NamedRunnable("gossip-watcher") {
      @Override
      public void execute() throws Exception {
  ...
  public void subscribe(String app, Consumer<Collection<String>> consumer) {
    executor.execute(new NamedRunnable("subscribe-app", app) {
      @Override
      public void execute() throws Exception {
  ...
  public void hash(Consumer<String> callback) {
    executor.execute(new NamedRunnable("compute-hash") {
      @Override
      public void execute() throws Exception {
  ...
  private void doBroadcast() {
    executor.execute(new NamedRunnable("gossip-broadcast") {
      @Override
      public void execute() throws Exception {
  ...
  private void gossipInExecutor() {
    ...
    StreamObserver<GossipReverse> interceptObserver = new StreamObserver<>() {
      ...
      public void onError(Throwable throwable) {
        executor.execute(new NamedRunnable("engine-reverse-on-error") {
          @Override
          public void execute() throws Exception {
            if (!finished) {
      ...
      public void onCompleted() {
        executor.execute(new NamedRunnable("engine-reverse-on-completed") {
          @Override
          public void execute() throws Exception {
            if (!finished) {
              metrics.gossips_inflight().down();
    ...
    executor.schedule(new NamedRunnable("gossip-initiate") {
      @Override
      public void execute() throws Exception {
  ...
  public void close() throws InterruptedException {
    if (alive.compareAndExchange(true, false) == true) {
      ...
      executor.execute(new NamedRunnable("gossip-closing") {
        @Override
        public void execute() throws Exception {
  ...
  public class Link {
    ...
    public Link(String target) {
      this.target = target;
      this.channel = NettyChannelBuilder.forTarget(target, credentials).withOption(ChannelOption.CONNECT_TIMEOUT_MILLIS, 1000).build();
