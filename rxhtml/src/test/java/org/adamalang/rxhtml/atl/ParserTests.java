...
public class ParserTests {
  ...
  public void simple() {
    ...
    Assert.assertEquals("\"xyz\"", node.js("$X"));
  ...
  public void variable() {
    ...
    Assert.assertEquals("\"hi \" + trim($X.first) + \" \" + $X.last", node.js("$X"));
    Set<String> vars = node.variables();
  ...
  public void condition_trailing() {
    ...
    Assert.assertEquals("\"hi \" + ($X.b) ? (\"active\") : (\"\")", node.js("$X"));
    Set<String> vars = node.variables();
  ...
  public void condition_trailing_negate() {
    ...
    Assert.assertEquals("\"hi \" + (!($X.b)) ? (\"inactive\") : (\"\")", node.js("$X"));
    Set<String> vars = node.variables();
    Assert.assertEquals(1, vars.size());
    Assert.assertTrue(vars.contains("b"));
  }
  ...
  public void condition() {
    ...
    Assert.assertEquals("\"hi \" + ($X.b) ? (\"A\") : (\"B\") + \" there\"", node.js("$X"));
    Set<String> vars = node.variables();
    Assert.assertEquals(1, vars.size());
    Assert.assertTrue(vars.contains("b"));
  }
