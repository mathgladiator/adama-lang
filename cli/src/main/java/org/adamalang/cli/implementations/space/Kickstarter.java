...
public class Kickstarter {
  ...
  public void flow() throws Exception {
    ...
    askSpaceName();
    if (space == null) {
      System.out.print(Util.prefix("Failed to get space name resolved", ANSI.Red));
      return;
    }
    File spaceDir = new File(space);
    if (spaceDir.exists()) {
      System.err.println(Util.prefix("The directory '" + space + "' already exists", ANSI.Yellow));
    }
    spaceDir.mkdirs();
    ...
    System.out.println("Directory '" + Util.prefix(space, ANSI.Cyan) + "' was created which contains a useful README.md\n");
    System.out.println("Your app is available online at https://" + space + ".adama.games/ \n");
    System.out.println(Util.prefix("Thank you for using Adama", ANSI.Green));
    System.out.println();
  }
  ...
  public void downloadAdama(File backendAdama) {
    try {
      ...
      if (ex instanceof ErrorCodeException) {
        System.err.println(Util.prefix("Failed downloading Adama Specification files (.adama):" + ((ErrorCodeException) ex).code, ANSI.Red));
      } else {
  ...
  public void downloadRxHTML(File rxhtmlAdama) {
    try {
      ...
      if (ex instanceof ErrorCodeException) {
        System.err.println(Util.prefix("Failed downloading RxHTML files (*.rx.html):" + ((ErrorCodeException) ex).code, ANSI.Red));
      } else {
        System.err.println("Exception: " + ex.getMessage());
      }
    }
  ...
  public void intro() {
    System.out.println("Greetings fellow human! Welcome to the " + Util.prefix("Adama Project Kickstart Tool", ANSI.Green) + "!");
    System.out.println();
  ...
  public void askTemplate() {
    ...
    System.out.print(Util.prefix("Template: ", ANSI.Cyan));
  ...
  public void askSpaceName() {
    ...
    while (true) {
      System.out.print(Util.prefix("Space name: ", ANSI.Cyan));
      this.space = System.console().readLine();
      try {
        ValidateSpace.validate(space);
      } catch (ErrorCodeException ex) {
        System.out.println(Util.prefix("Invalid space name (reason=" + ex.code + ")!", ANSI.Red) + " Try again please.");
        System.out.println();
        continue;
      }
      System.out.println();
      ...
      try {
        ...
        System.out.println();
        System.out.println(Util.prefix("Space created!", ANSI.Green));
        System.out.println();
        return;
      } catch (Exception ex) {
        if (ex instanceof ErrorCodeException) {
          if (((ErrorCodeException) ex).code == 130092 || ((ErrorCodeException) ex).code == 667658) {
            System.err.println(Util.prefix("This space already exists!", ANSI.Red));
            System.err.println(Util.prefix("Do you want to use it anyway and setup your local project anyway? [yes/no]", ANSI.Yellow));
          ...
          System.err.println(Util.prefix("Failed:" + ((ErrorCodeException) ex).code + "!!", ANSI.Red));
        } else {
          System.err.println("Exception: " + ex.getMessage());
          return;
        }
