...
public class CodeGenDeltaClass {
  private static boolean canFieldBeSeenAtSomePoint(final FieldDefinition fd) {
    if (fd.policy == null || fd.policy instanceof PrivatePolicy) {
      return false;
    }
    return true;
  }
  ...
  public static void writeMessageDeltaClass(
  ...
  public static void writeRecordDeltaClass(
      final StructureStorage storage,
      final StringBuilderWithTabs sb,
      final Environment environment,
      final String className,
    ...
    for (final FieldDefinition fd : storage.fieldsByOrder) {
      ...
      if (canFieldBeSeenAtSomePoint(fd)) {
        ...
        sb.append("private ")
            .append(((DetailHasDeltaType) fieldType).getDeltaType(environment))
            .append(" __d")
            .append(fd.name)
            .append(";")
            .writeNewline();
    ...
    for (final BubbleDefinition bd : storage.bubbles.values()) {
      ...
      sb.append("private ")
          .append(((DetailHasDeltaType) bubbleType).getDeltaType(environment))
          .append(" __d")
          .append(bd.nameToken.text)
          .append(";")
          .writeNewline();
    ...
    if (storage.bubbles.size() > 0) {
      sb.append("RTx__ViewerType __VIEWER = (RTx__ViewerType) __writer.viewerState;")
          .writeNewline();
      sb.append("long __CHECK = 0;").writeNewline();
      for (final BubbleDefinition bd : storage.bubbles.values()) {
        sb.append("__CHECK = __item.___")
            .append(bd.nameToken.text)
            .append(".getGeneration() * 1662803L + __VIEWER.__DATA_GENERATION;")
            .writeNewline();
        sb.append("if (__g")
            .append(bd.nameToken.text)
            .append(" != __CHECK)  {")
            ...
            .writeNewline();
        final var bubbleType = environment.rules.Resolve(bd.expressionType, false);
        sb.append(bubbleType.getJavaBoxType(environment))
            .append(" __local_")
            .append(bd.nameToken.text)
            .append(" = __item.__COMPUTE_")
            .append(bd.nameToken.text)
            .append("(__writer.who, __VIEWER);")
            .writeNewline();
        writeShowData(
            sb,
            "__d" + bd.nameToken.text,
            "__local_" + bd.nameToken.text,
            bubbleType,
            "__obj.planField(\"" + bd.nameToken.text + "\")",
            environment,
            false);
  ...
  private static void writeShowData(
      final StringBuilderWithTabs sb,
      final String deltaObject,
      final String sourceData,
      final TyType sourceType,
      final String targetObjectWriter,
      final Environment environment,
      final boolean tabDown) {
    if (sourceType instanceof TySimpleNative
        || sourceType instanceof TyReactiveRecord
        || sourceType instanceof TyNativeMessage) {
      sb.append(deltaObject)
          .append(".show(")
          .append(sourceData)
          .append(", ")
          .append(targetObjectWriter)
          .append(");");
      if (tabDown) {
        sb.tabDown();
      }
      ...
      final var elementType =
          ((DetailContainsAnEmbeddedType) sourceType).getEmbeddedType(environment);
      if (elementType instanceof TyReactiveRecord) {
        writeShowDListRecords(
            sb, deltaObject, sourceData, elementType, targetObjectWriter, environment, tabDown);
      } else {
        writeShowDListNonRecord(
            sb, deltaObject, sourceData, elementType, targetObjectWriter, environment, tabDown);
      }
      ...
      writeShowDMaybe(
          ...
          deltaObject,
          sourceData,
          ((TyNativeMaybe) sourceType).getEmbeddedType(environment),
          targetObjectWriter,
          environment,
          tabDown);
      ...
      writeShowDMap(
          ...
          deltaObject,
          sourceData,
          ((TyNativeMap) sourceType).domainType,
          environment.rules.Resolve(((TyNativeMap) sourceType).rangeType, false),
          targetObjectWriter,
          environment,
          tabDown);
      ...
      writeShowDMapRx(
          ...
          deltaObject,
          sourceData,
          ((TyReactiveMap) sourceType).getDomainType(environment),
          walkRangeType,
          rangeType,
          addGet,
          targetObjectWriter,
  ...
  private static void writeShowDListNonRecord(
      final StringBuilderWithTabs sb,
      final String deltaObject,
      final String sourceData,
      final TyType elementType,
      final String targetObjectWriter,
      final Environment environment,
      final boolean tabDown) {
    ...
    sb.append("DList<")
        ...
        .append(";")
        .writeNewline();
    sb.append("PrivateLazyDeltaWriter ")
        .append(listWriterVar)
        .append(" = ")
        .append(targetObjectWriter)
        .append(".planObject();")
        .writeNewline();
    ...
    sb.append("for (")
        .append(childElementType)
        .append(" ")
        .append(childElementVar)
        .append(" : ")
        .append(sourceData)
        .append(") {")
        .tabUp()
        .writeNewline();
    sb.append(elementDeltaType)
        .append(" ")
        .append(childDeltaVar)
        .append(" = ")
        .append(dListCache)
        .append(".getPrior(")
        .append(indexVar)
    ...
    writeShowData(
        ...
        childElementVar,
        elementType,
        listWriterVar + ".planField(" + indexVar + ")",
        environment,
        false);
    sb.append(indexVar).append("++;").tabDown().writeNewline();
    sb.append("}").writeNewline();
    sb.append(dListCache)
        .append(".rectify(")
        .append(indexVar)
        .append(", ")
        .append(listWriterVar)
  ...
  private static void writeShowDListRecords(
      final StringBuilderWithTabs sb,
      final String deltaObject,
      final String sourceData,
      final TyType elementType,
    ...
    writeShowData(
        ...
        childElementVar,
        elementType,
        listWriterVar + ".planField(" + childElementVar + ".__id())",
        environment,
        false);
    sb.append(dListWalker)
        .append(".next(")
        .append(childElementVar)
        .append(".__id());")
        .tabDown()
  ...
  private static void writeShowDMap(
      final StringBuilderWithTabs sb,
      final String deltaObject,
      final String sourceData,
    ...
    final var entryType =
        "Map.Entry<"
            ...
            + ","
            + rangeType.getJavaBoxType(environment)
            + ">";
    ...
    sb.append("PrivateLazyDeltaWriter ")
        ...
        .writeNewline();
    sb.append("DMap<")
        .append(domainBoxType)
        .append(",")
        .append(rangeDeltaType)
        .append("> ")
        .append(dMapCache)
        .append(" = ")
        .append(deltaObject)
    ...
    writeShowData(
        ...
        mapEntry + ".getValue()",
        rangeType,
        listWriterVar + ".planField(\"\" + " + mapEntry + ".getKey())",
        environment,
        true);
    sb.append("}").writeNewline();
  ...
  private static void writeShowDMapRx(
      final StringBuilderWithTabs sb,
      final String deltaObject,
      final String sourceData,
      final TyType domainType,
      final TyType walkRangeType,
      final TyType getRangeType,
      boolean addGet,
      final String targetObjectWriter,
      final Environment environment,
      final boolean tabDown) {
    ...
    final var dMapCache = "__deltaMap" + environment.autoVariable();
    final var dMapCacheWalker = "__deltaMapWalker" + environment.autoVariable();
    final var entryTypeWalk =
        "Map.Entry<"
            + domainType.getJavaBoxType(environment)
            + ","
            + walkRangeType.getJavaBoxType(environment)
            + ">";
    ...
    writeShowData(
        sb,
        childDeltaVar,
        mapEntryWalk + ".getValue()" + (addGet ? ".get()" : ""),
        getRangeType,
        listWriterVar + ".planField(\"\" + " + mapEntryWalk + ".getKey())",
        environment,
        true);
    sb.append("}").writeNewline();
    sb.append(dMapCacheWalker).append(".end(").append(listWriterVar).append(");").writeNewline();
  ...
  private static void writeShowDMaybe(
      final StringBuilderWithTabs sb,
      final String deltaObject,
      final String sourceData,
      final TyType elementType,
    ...
    sb.append(elementDeltaType)
        ...
        .writeNewline();
    writeShowData(
        sb, deltaElementVar, childElementVar, elementType, targetObjectWriter, environment, true);
    sb.append("} else {").tabUp().writeNewline();
    sb.append(deltaObject)
        .append(".hide(")
        .append(targetObjectWriter)
