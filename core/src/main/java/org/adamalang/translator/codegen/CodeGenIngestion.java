...
public class CodeGenIngestion {
  public static void writeJava(
      ...
      if (isArray) {
        ...
        original.define(
            ...
            new TyNativeArray(
                ...
                new TyNativeInteger(TypeBehavior.ReadOnlyNativeValue, exportIdsAs, exportIdsAs),
                exportIdsAs),
            true,
            assignment);
      } else {
    ...
    if (exportIdsAs != null && isArray) {
      ...
      sb.append("ArrayList<Integer> ")
          .append(generatedIntArrayVar)
          .append(" = new ArrayList<Integer>();")
          .writeNewline();
    }

    final var generatedRefVariable = "_AutoRef" + environment.autoVariable();
    sb.append(refType.getJavaConcreteType(environment))
        .append(" ")
        .append(generatedRefVariable)
        .append(" = ");
    ...
    if (isMap) {
      TyType inputDomainType =
          environment.rules.Resolve(((IsMap) exprType).getDomainType(environment), true);
      TyType inputRangeType =
          environment.rules.Resolve(((IsMap) exprType).getRangeType(environment), true);
      TyType outputRangeType =
          environment.rules.Resolve(((IsMap) refType).getRangeType(environment), true);
      final var generatedExprVariableEntry = "_AutoEntry" + autoVar;
      sb.append("for (Map.Entry<")
          .append(inputDomainType.getJavaBoxType(environment))
          .append(",")
          .append(inputRangeType.getJavaBoxType(environment))
          .append("> ")
          .append(generatedExprVariableEntry);
      sb.append(" : (");
      assignment.expression.writeJava(
          sb, environment.scopeWithComputeContext(ComputeContext.Computation));
      sb.append(").entries()) {").tabUp().writeNewline();
      generateAssignType =
          outputRangeType.makeCopyWithNewPosition(refType, TypeBehavior.ReadWriteNative);
      final var generatedExprVariable = "_AutoExpr" + autoVar;
      generateAssignVar = "_AutoRefElement" + autoVar;
      sb.append(inputRangeType.getJavaConcreteType(environment))
          .append(" ")
          .append(generatedExprVariable)
          .append(" = ")
          .append(generatedExprVariableEntry)
          .append(".getValue();")
          .writeNewline();
      sb.append(generateAssignType.getJavaConcreteType(environment))
          .append(" ")
          .append(generateAssignVar)
          .append(" = ");
      sb.append(generatedRefVariable)
          .append(".getOrCreate(")
          .append(generatedExprVariableEntry)
          .append(".getKey());")
          .writeNewline();
      ...
      sb.append("for (");
      sb.append(iterateElementType.getJavaConcreteType(environment))
          .append(" ")
          .append(generatedExprVariable)
          .append(" : ");
      assignment.expression.writeJava(
      ...
      if (hasId) {
        generateAssignType =
            environment.rules.Resolve(environment.rules.ExtractEmbeddedType(refType, true), true)
                .makeCopyWithNewPosition(refType, TypeBehavior.ReadWriteNative);
        ...
        sb.append(generateAssignType.getJavaConcreteType(environment))
            ...
            .writeNewline();
        if (exportIdsAs != null && isArray) {
          sb.append(generatedIntArrayVar)
              .append(".add(")
              .append(generateAssignVar)
              .append(".id.get());")
              .writeNewline();
      ...
      if (exportIdsAs != null) {
        sb.writeNewline();
        sb.append(exportIdsAs.text)
            .append(" = Utility.convertIntegerArrayList(")
            .append(generatedIntArrayVar)
            .append(");");
      }
      ...
      sb.append(exprType.getJavaConcreteType(environment))
          .append(" ")
          .append(generatedExprVariable)
          .append(" = ");
      assignment.expression.writeJava(
      ...
      finish(
          environment, sb, generateAssignType, generateAssignVar, exprType, generatedExprVariable);
    }
    sb.append("}");
  }
  ...
  private static void finish(
      ...
      final StringBuilderWithTabs sb,
      final TyType assignTypeRaw,
      final String assignVar,
      final TyType elementTypeRaw,
    ...
    if (assignVar != null && elementTypeRaw != null) {
      ...
      if (environment.rules.IsStructure(assignType, true)
          && environment.rules.IsStructure(elementType, true)) {
        ...
        if (!environment.state.hasNoCost()) {
          sb.append("__code_cost += ")
              .append(Integer.toString(((IsStructure) elementType).storage().fields.size()))
              .append(";")
              .writeNewline();
        }
        for (final Map.Entry<String, FieldDefinition> entryType :
            ((IsStructure) elementType).storage().fields.entrySet()) {
          final var fd = ((IsStructure) assignType).storage().fields.get(entryType.getKey());
          if (fd != null) {
            final var leftAssignType =
                ((IsStructure) assignType).storage().fields.get(entryType.getKey()).type;
            final var op =
                environment.rules.IngestionLeftElementRequiresRecursion(leftAssignType)
                    ? "<-"
                    : "=";
            final var ass =
                new Assignment( //
                    new FieldLookup(
                        new Lookup(Token.WRAP(assignVar)), null, entryType.getValue().nameToken),
                    Token.WRAP(op), //
                    new FieldLookup(
                        new Lookup(Token.WRAP(elementVar)), null, entryType.getValue().nameToken),
                    null,
                    null,
                    null,
                    false);
        ...
        final var ass =
            new Assignment( //
                new Lookup(Token.WRAP(assignVar)),
                Token.WRAP("="), //
                new Lookup(Token.WRAP(elementVar)),
                null,
                null,
                null,
                false);
