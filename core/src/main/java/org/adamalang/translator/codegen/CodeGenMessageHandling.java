...
public class CodeGenMessageHandling {
  public static void writeMessageHandlers(
    ...
    for (final DefineHandler handler : environment.document.handlers) {
      if (channelsDefined.contains(handler.channel)) {
        environment.document.createError(
            handler,
            String.format("Channel '%s' is already handled", handler.channel),
            "MessageHandlers");
        continue;
      }
      ...
      final IsStructure rawStorageType =
          environment.rules.FindMessageStructure(handler.typeName, handler, false);
      if (rawStorageType != null && rawStorageType instanceof TyNativeMessage) {
        ...
        if (handler.isArray) {
          ...
          parser2
              .append("ArrayList<RTx")
              .append(handler.typeName)
              .append("> __array_")
              .append(handler.channel)
          ...
          parser2
              .append("return __array_")
              .append(handler.channel)
              .append(".toArray(new RTx")
              .append(handler.typeName)
              .append("[__array_")
              .append(handler.channel)
              .append(".size()]);")
        ...
        if (handler.behavior == MessageHandlerBehavior.EnqueueItemIntoNativeChannel) {
          ...
          final var mapToIndexIn =
              handler.isArray ? channelsPerMessageArrayType : channelsPerMessageType;
          ...
          classFields
              .append("> ")
              .append(handler.channel)
              .append(" = new NtChannel<>(__futures, __queue_" + handler.channel + ");")
              .writeNewline();
        ...
        if (handler.behavior == MessageHandlerBehavior.ExecuteAssociatedCode) {
          dispatch.append("case \"").append(handler.channel).append("\":").tabUp().writeNewline();
          dispatch
              .append("__task.setAction(() -> handleChannelMessage_")
              .append(handler.channel)
              .append("(__task.who, (RTx")
              .append(handler.typeName);
          if (handler.isArray) {
            dispatch.append("[]");
          }
          ...
          sb.append("private void handleChannelMessage_")
              .append(handler.channel)
              .append("(NtClient ")
              .append(clientVarToUse)
              .append(", RTx")
              .append(handler.typeName);
          if (handler.isArray) {
            sb.append("[]");
          ...
          sb.append(" ")
              .append(payloadNameToUse)
              .append(") throws AbortMessageException {")
              .tabUp()
              .writeNewline();
    ...
    if (environment.document.handlers.size() > 0) {
      ...
      sb.append("@Override").writeNewline();
      sb.append("protected Object __parse_message2(String __channel, JsonStreamReader __reader) {")
          .tabUp()
          .writeNewline();
      ...
      sb.append("}").writeNewline();
      sb.append("@Override").writeNewline();
      sb.append("protected Object __parse_message2(String channel, JsonStreamReader __reader) {")
          .tabUp()
          .writeNewline();
      sb.append("__reader.skipValue();").writeNewline();
