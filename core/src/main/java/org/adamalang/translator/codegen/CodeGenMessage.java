...
public class CodeGenMessage {
  public static void generateJsonReaders(
      final String name,
    ...
    { // READ FROM STREAM
      sb.append("private RTx" + name + "(JsonStreamReader __reader) {")
          .tabUp()
          .writeNewline(); // UP
      ...
      if (storage.fields.size() == 0) {
        ...
        for (final Map.Entry<String, FieldDefinition> e : storage.fields.entrySet()) {
          ...
          writeValueReader(
              "this." + e.getKey(),
              environment.rules.Resolve(e.getValue().type, false),
              sb,
              environment,
              localVar);
  ...
  public static void writeValueReader(
      final String name,
      final TyType type,
      final StringBuilderWithTabs sb,
      final Environment environment,
      final AtomicInteger localVar) {
    if (type instanceof TySimpleNative) {
      sb.append(name)
          .append(" = ")
          .append("__reader.read")
          .append(type.getJavaBoxType(environment))
          .append("();")
          .writeNewline();
      ...
      sb.append(elementType.getJavaConcreteType(environment))
          .append(" ")
          .append(localItem)
          .append(";")
          .writeNewline();
      ...
      sb.append(name)
          .append(" = new RTx")
          .append(((TyNativeMessage) type).nameToken.text)
          .append("(__reader);")
          .writeNewline();
  ...
  public static void writeValueWriter(
      final String name,
      final TyType type,
      final StringBuilderWithTabs sb,
      final Environment environment,
    ...
    if (type instanceof TySimpleNative) {
      ...
      sb.append("for (Map.Entry<")
          .append(domainType.getJavaBoxType(environment))
          .append(", ")
          .append(rangeType.getJavaBoxType(environment))
          .append("> ")
          .append(localItem)
          .append(" : ")
          .append(name)
