...
public class TyNativeChannel extends TyType implements DetailTypeHasMethods, DetailContainsAnEmbeddedType, AssignmentViaNative {
  ...
  public TyNativeChannel(final Token channelToken, final TokenizedItem<TyType> tokenizedType) {
  ...
  public TyNativeFunctional lookupMethod(final String name, final Environment environment) {
    ...
    final var ct = new TyNativeClient(null);
    final var argTypes = new ArrayList<TyType>();
    if (environment.rules.IsNativeArray(resolvedChannelType, true)) {
      if ("choose".equals(name)) {
        final var maft = new TyNativeFuture(null, new TokenizedItem<>(new TyNativeMaybe(null, new TokenizedItem<>(resolvedChannelType)).withPosition(this)));
        final var limit = new TyNativeInteger(Token.WRAP("int"));
      ...
      if ("fetch".equals(name)) {
        final var ft = new TyNativeFuture(null, new TokenizedItem<>(resolvedChannelType.withPosition(this)));
        argTypes.add(ct.withPosition(this));
      ...
      if ("decide".equals(name)) {
        final var mft = new TyNativeFuture(null, new TokenizedItem<>(new TyNativeMaybe(null, new TokenizedItem<>(resolvedChannelType)).withPosition(this)));
        argTypes.add(ct.withPosition(this));
        argTypes.add(new TyNativeArray(resolvedChannelType, Token.WRAP("[]")).withPosition(this));
  ...
  public TyType makeCopyWithNewPosition(final DocumentPosition position) {
    return new TyNativeChannel(channelToken, tokenizedType).withPosition(position);
  }
