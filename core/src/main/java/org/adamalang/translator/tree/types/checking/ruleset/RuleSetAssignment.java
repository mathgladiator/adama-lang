...
public class RuleSetAssignment {
  ...
  public static CanAssignResult CanAssignWithSet(final Environment environment, final TyType left, final TyType right, final boolean silent) {
    ...
    var result = TestAssignReactively(environment, left, right);
    if (result != CanAssignResult.No) { return result; }
    if (left instanceof AssignmentViaNative || left instanceof AssignmentViaNativeOnlyForSet) { return CanAssignResult.YesWithNativeOp; }
    result = CanAssignBase(environment, left, right, silent);
    if (result != CanAssignResult.No) { return result; }
  ...
  public static boolean CanTypeAStoreTypeB(final Environment environment, final TyType originalTypeA, final TyType originalTypeB, final StorageTweak tweak, final boolean silent) {
    ...
    if (tweak == StorageTweak.Add) {
      if (aString && (bInteger || bBoolean || bDouble)) { return true; }
    }
    ...
    if (aEnum && bEnum) {
      if (((IsEnum) typeA).name().equals(((IsEnum) typeB).name())) { return true; }
      if (!silent) {
    ...
    if (aMap && bMap) {
      ...
      if (ab1 && ab2 && ab3 && ab4) { return true; }
    }
    ...
    if (aTable && bTable) { return ((TyNativeTable) typeA).messageName.equals(((TyNativeTable) typeB).messageName); }
    ...
    if (aMaybe && bEmbedAssign == AssignableEmbedType.None) {
      ...
      final var childA = RuleSetCommon.ExtractEmbeddedType(environment, typeA, silent);
      if (CanTypeAStoreTypeB(environment, childA, typeB, tweak, silent)) { return true; }
    }
  ...
  private static AssignableEmbedType TestAssignableWithEmbedd(final TyType type) {
    if (type instanceof TyNativeMaybe || type instanceof TyReactiveMaybe) { return AssignableEmbedType.Maybe; }
    if (type instanceof TyNativeArray) { return AssignableEmbedType.Array; }
    if (type instanceof TyNativeList) { return AssignableEmbedType.List; }
    if (type instanceof TyNativeFuture) { return AssignableEmbedType.Future; }
    if (type instanceof TyNativeReactiveRecordPtr) { return AssignableEmbedType.Ptr; }
