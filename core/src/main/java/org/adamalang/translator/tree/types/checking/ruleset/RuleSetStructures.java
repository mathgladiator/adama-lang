...
public class RuleSetStructures {
  ...
  public static boolean CanStructureAProjectIntoStructureB(
      final Environment environment, final TyType typeA, final TyType typeB, final boolean silent) {
    var result = false;
    if (typeA != null
        && typeB != null
        && typeA instanceof IsStructure
        && typeB instanceof IsStructure) {
      result = true;
      ...
      for (final Map.Entry<String, FieldDefinition> elementB : storB.fields.entrySet()) {
        ...
        if (other != null) {
          RuleSetAssignment.CanTypeAStoreTypeB(
              environment, elementB.getValue().type, other.type, StorageTweak.None, silent);
        } else {
          if (!silent) {
            environment.document.createError(
                typeA,
                String.format(
                    "The type '%s' contains field '%s' which is not found within '%s'.",
                    typeB.getAdamaType(), elementB.getKey(), typeA.getAdamaType()),
                "RuleSetStructures");
          }
          result = false;
        }
  ...
  public static boolean IsStructure(
    ...
    if (tyType != null) {
      ...
      if (tyType != null && (tyType instanceof IsStructure)) {
        ...
        environment.document.createError(
            tyTypeOriginal,
            String.format(
                "Type check failure: must have a type of 'record' or 'message', but got a type of '%s'.",
