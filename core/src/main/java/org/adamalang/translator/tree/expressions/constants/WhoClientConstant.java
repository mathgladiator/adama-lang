...
public class WhoClientConstant extends Expression {
  ...
  protected TyType typingInternal(final Environment environment, final TyType suggestion) {
    if (environment.state.isStatic() || environment.state.isMessageHandler() || environment.state.isDocumentPolicy()) {
      environment.mustBeComputeContext(this);
      return new TyNativeClient(TypeBehavior.ReadOnlyNativeValue, null, token).withPosition(this);
    } else {
      environment.document.createError(this, "@who is only available from static policies, document policies, and message handlers", "WHO");
      return null;
    }
  }
