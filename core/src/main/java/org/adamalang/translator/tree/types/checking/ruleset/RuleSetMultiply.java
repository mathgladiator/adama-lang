...
public class RuleSetMultiply {
  public static CanMathResult CanMultiply(
    ...
    if (typeA != null && typeB != null) {
      ...
      if (aLong && bLong || aLong && bInteger || aInteger && bLong) {
        return CanMathResult.YesAndResultIsLong;
      }
      final var aString = RuleSetCommon.IsString(environment, typeA, true);
      if (aString && bInteger) {
        return CanMathResult.YesAndResultIsStringRepetitionUsingSpecialMultiplyOp;
      }
      ...
      if (aDouble && bDouble || aDouble && (bInteger || bLong) || (aInteger || aLong) && bDouble) {
