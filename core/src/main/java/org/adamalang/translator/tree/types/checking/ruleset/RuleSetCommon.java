...
public class RuleSetCommon {
  static boolean AreBothChannelTypesCompatible(
      final Environment environment, final TyType typeA, final TyType typeB) {
    ...
    if (resolveA != null && resolveB != null) {
      ...
      if (aChannel && bChannel) {
        ...
        if (aChildType.getAdamaType().equals(bChildType.getAdamaType())) {
          return true;
        }
      }
    }
  ...
  public static TyType EnsureRegisteredAndDedupe(
      final Environment environment, final TyType type, final boolean silent) {
    ...
    if (type != null && environment.rules.IsNativeMessage(type, true)) {
      ...
      if (msg.storage.anonymous) {
        for (final Map.Entry<String, FieldDefinition> entry : msg.storage.fields.entrySet()) {
          entry.getValue().type =
              EnsureRegisteredAndDedupe(environment, entry.getValue().type, silent);
        }
  ...
  public static TyType GetMaxType(
    ...
    final var aReactive =
        RuleSetCommon.TestReactive(resolveA) && resolveA instanceof DetailComputeRequiresGet;
    final var bReactive =
        RuleSetCommon.TestReactive(resolveB) && resolveB instanceof DetailComputeRequiresGet;
    if (aReactive && bReactive) {
      if (RuleSetEquality.CanTestEquality(
              environment,
              ((DetailComputeRequiresGet) resolveA).typeAfterGet(environment),
              ((DetailComputeRequiresGet) resolveB).typeAfterGet(environment),
              false)
          != CanTestEqualityResult.No) {
        return resolveA
            .makeCopyWithNewPosition(typeB, TypeBehavior.ReadOnlyNativeValue)
            .withPosition(typeA);
      }
    }
    ...
    if (RuleSetEquality.CanTestEquality(environment, resolveA, resolveB, true)
        != CanTestEqualityResult.No) {
      return resolveA
          .makeCopyWithNewPosition(typeB, TypeBehavior.ReadOnlyNativeValue)
          .withPosition(typeA);
    ...
    if (aMessage && bMessage) {
      final var nextMessageType =
          messageUnion(environment, (TyNativeMessage) resolveA, (TyNativeMessage) resolveB, silent);
    ...
    if (aMaybe || bMaybe) {
      ...
      if (maxType != null) {
        return new TyNativeMaybe(
            TypeBehavior.ReadOnlyNativeValue, null, null, new TokenizedItem<>(maxType));
      }
    }
  ...
  public static WrapInstruction GetMaxTypeBasedWrappingInstruction(
      final Environment environment, final TyType typeA, final TyType typeB) {
    final var resolveA = Resolve(environment, typeA, true);
    final var resolveB = Resolve(environment, typeB, true);
    if (resolveA == null || resolveB == null) {
  ...
  static void SignalTypeNotFound(
      final Environment environment, final TyType type, final boolean silent) {
    if (!silent) {
      environment.document.createError(
          type,
          String.format("Type not found: the type '%s' was not found.", type.getAdamaType()),
          "TypeCheckReferences");
    }
  }
  ...
  static void SignalTypeFailure(
      final Environment environment,
      final TyType expected,
      final TyType given,
    ...
    if (!silent && given != null) {
      environment.document.createError(
          given,
          String.format(
              "Type check failure: must have a type of '%s', but the type is actually '%s'",
              expected.getAdamaType(), given.getAdamaType()),
          "TypeCheckFailures");
    }
  }
  ...
  static boolean IsDouble(
    ...
    if (tyType != null) {
      ...
      SignalTypeFailure(
          environment,
          new TyNativeDouble(TypeBehavior.ReadOnlyNativeValue, null, null),
          tyTypeOriginal,
          silent);
    }
    return false;
  ...
  public static boolean IsNumeric(
    ...
    if (tyType != null) {
      if (tyType instanceof TyNativeInteger
          || tyType instanceof TyReactiveInteger
          || tyType instanceof TyNativeDouble
          || tyType instanceof TyReactiveDouble) {
        return true;
      }
      if (!silent && tyTypeOriginal != null) {
        environment.document.createError(
            tyTypeOriginal,
            String.format(
                "Type check failure: Must have a type of '%s' or '%s', but the type is actually '%s'", //
                new TyNativeInteger(TypeBehavior.ReadOnlyNativeValue, null, null)
                    .getAdamaType(), //
                new TyNativeDouble(TypeBehavior.ReadOnlyNativeValue, null, null).getAdamaType(),
  ...
  static boolean IsString(
    ...
    if (tyType != null) {
      ...
      SignalTypeFailure(
          environment,
          new TyNativeString(TypeBehavior.ReadOnlyNativeValue, null, null),
          tyTypeOriginal,
          silent);
    }
    return false;
  ...
  static boolean IsDynamic(
    ...
    if (tyType != null) {
      ...
      SignalTypeFailure(
          environment,
          new TyNativeDynamic(TypeBehavior.ReadOnlyNativeValue, null, null),
          tyTypeOriginal,
          silent);
    }
    return false;
  ...
  static boolean IsAsset(
    ...
    if (tyType != null) {
      ...
      SignalTypeFailure(
          environment,
          new TyNativeAsset(TypeBehavior.ReadOnlyNativeValue, null, null),
          tyTypeOriginal,
          silent);
    }
    return false;
  ...
  private static TyType messageUnion(
      final Environment environment,
      final TyNativeMessage aActualMessage,
      final TyNativeMessage bActualMessage,
      final boolean silent) {
    if (aActualMessage == bActualMessage
        || aActualMessage.storage == bActualMessage.storage
        || aActualMessage.name.equals(bActualMessage.name)) { // they are the same
    ...
    if (aActualMessage.storage.anonymous) {
      if (bActualMessage.storage.anonymous) {
        ...
        for (final Map.Entry<String, FieldDefinition> aEntry :
            aActualMessage.storage.fields.entrySet()) {
          final var bFd = bActualMessage.storage.fields.get(aEntry.getKey());
          if (bFd != null) {
            newStorage.fields.put(
                aEntry.getKey(),
                new FieldDefinition(
                    null,
                    null,
                    GetMaxType(environment, aEntry.getValue().type, bFd.type, false),
                    bFd.nameToken,
                    null,
                    null,
                    null,
        ...
        for (final Map.Entry<String, FieldDefinition> bEntry :
            bActualMessage.storage.fields.entrySet()) {
        ...
        return new TyNativeMessage(
            ...
            null,
            Token.WRAP("AutoMaxRecord" + environment.autoVariable()),
            newStorage);
      } else {
        if (environment.rules.CanStructureAProjectIntoStructureB(
            bActualMessage, aActualMessage, silent)) {
      ...
      if (bActualMessage.storage.anonymous) {
        if (environment.rules.CanStructureAProjectIntoStructureB(
            aActualMessage, bActualMessage, silent)) {
          return aActualMessage;
        }
      } else if (!silent) {
        environment.document.createError(
            aActualMessage,
            String.format(
                "The message types `%s` and `%s` can not be joined.",
                aActualMessage.getAdamaType(), bActualMessage.getAdamaType()),
            "MaxUnionType");
      }
    }
    return null;
  ...
  public static TyType ResolvePtr(
      final Environment environment, final TyType tyTypeOriginal, final boolean silent) {
  ...
  static void SignalTypeCompatibility(
      final Environment environment,
      final TyType expected,
      final TyType given,
      final boolean silent) {
    if (!silent && given != null) {
      environment.document.createError(
          given,
          String.format(
              "Type check failure: The types '%s' and '%s' are not compatible for type unification",
              expected.getAdamaType(), given.getAdamaType()),
          "TypeCompatabilities");
    }
  }
  ...
  static boolean TestReactive(final TyType tyType) {
    if (tyType != null) {
      if (tyType instanceof IsReactiveValue
          || tyType instanceof TyReactiveRecord
          || tyType instanceof TyReactiveTable
          || tyType instanceof TyReactiveMaybe
          || tyType instanceof TyReactiveLazy) {
        return true;
      }
    }
    return false;
