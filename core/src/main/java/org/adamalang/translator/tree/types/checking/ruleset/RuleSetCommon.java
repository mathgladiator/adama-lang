      ...
      if (bActualMessage.storage.anonymous) {
        ...
        for (final Map.Entry<String, FieldDefinition> aEntry : aActualMessage.storage.fields.entrySet()) {
          ...
          if (bFd != null) {
            ...
            policy.ingest(bFd);
            toAdd = new FieldDefinition(policy, null, GetMaxType(environment, aEntry.getValue().type, bFd.type, false), bFd.nameToken, null, null, null, null, null, null, null, null);
            toAdd.ingest(bFd);
          } else {
