  ...
  public void flow() throws Exception {
    ...
    try (DataBase dataBase = new DataBase(dataBaseConfig, new DataBaseMetrics(new NoOpMetricsFactory()))) {
      ...
      try {
        ...
        Assert.assertTrue(Domains.map(dataBase, 141, "www.domain.com", "my-space", null, "certificate-yay"));
        ...
        Assert.assertFalse(Domains.map(dataBase, 145, "www.domain.com", "my", null, "yo"));
        result = Domains.get(dataBase, "www.domain.com");
        Assert.assertEquals("my-space", result.space);
        Assert.assertEquals("certificate-yay", result.certificate);
        Assert.assertEquals(141, result.owner);
        Assert.assertTrue(Domains.map(dataBase, 141, "www.domain.com", "my-space-two", null,"certificate-ok"));
        ...
        result = Domains.get(dataBase, "www.domain.com");
        Assert.assertNull(result);
        Assert.assertTrue(Domains.map(dataBase, 42, "www.domain1.com", "space", null,"certificate-ok"));
        Assert.assertTrue(Domains.map(dataBase, 42, "www.domain2.com", "space", "some-doc","certificate-ok"));
        Assert.assertEquals("some-doc", Domains.get(dataBase, "www.domain2.com").key);
        Assert.assertTrue(Domains.map(dataBase, 42, "www.domain3.com", "space", null,"certificate-ok"));
        Assert.assertEquals(3, Domains.deleteSpace(dataBase, "space"));
        Assert.assertTrue(Domains.map(dataBase, 500, "www.my-domain.com", "space", "over",null));
        Assert.assertTrue(Domains.map(dataBase, 500, "www.my-domain.com", "space2", null,null));
