# Language Guide

In [static policies and document events](./static-policies-document-events.md), you will learn how to authorize and allow people to connect to documents.


- [Document layout with types](./document-layout-and-types.md)
- [Comments are good for your health](./comments.md)
- [Constants](./constants.md)
- [Records and Messages](./records-and-messages.md)
- [Local variables and assignment](./local-variables-and-assignment.md)
- [Doing math](./math.md)
- [Anonymous messages and arrays](./anonymous.md)
- [Maybe some data, maybe not](./maybe.md)
- [Tables and integrated query](./tables-linq.md)
- [Maps and reduce](./map-reduce.md)
- [Standard control](./control.md)
- [Functions, procedures, and methods oh my](./functions-procedures-methods-oh-my.md)
- [Enumerations and dynamic dispatch](./enumerations.md)
- [Async with channels, futures, and handlers](./async.md)
- [The glorious state machine](./state-machine.md)
- [Reactive formulas](./formulas.md)
- [Privacy and bubbles](./privacy-and-bubbles.md)
- [Web processing](./web.md)
- [Interacting with remote services](./services.md)