...
### Template
```js
connection.RegionalDomainLookup(identity, domain, {
  success: function(response) {
    // response.domain
    // response.owner
    // response.space
...
| --- | --- | --- |
| domain | String | A domain name. |
| owner | Integer | Adama ID for the owner of an asset (encoded email) |
| space | String | A space which is a collection of documents with a common Adama schema. |
| key | String | The key. |
| route | Boolean | Does the domain route GET to the document or the space. |
| certificate | String | The certificate of a domain |
| timestamp | Long | A system timestamp for actions |

## Method: RegionalFinderFind
...
### Template
```js
connection.RegionalFinderFind(identity, space, key, {
  success: function(response) {
    // response.id
...
| --- | --- | --- |
| id | Long | a long id |
| location-type | Integer | Information about the found results |
| archive | String | The archive containing the backup of the document |
| region | String | The region where the document exists |
| machine | String | The machine the document is located on |
| deleted | Boolean | Is the document marked for deletion? |

## Method: RegionalFinderFree
...
| space | yes | String | A 'space' is a collection of documents with the same schema and logic, and the 'space' parameter is used to             denote the name of that collection.              Spaces are lower case ASCII using the regex a-z[a-z0-9\-]* to validation with a minimum length of three characters. The space name must also not contain a '--' |
| key | yes | String | Within a space, documents are organized within a map and the 'key' parameter will uniquely identify             documents.              Keys are lower case ASCII using the regex [a-z0-9\._\-]* for validation |
| region | yes | String | The region to bind too |
| machine | yes | String | The machine to bind too |


### Template
```js
connection.RegionalFinderFree(identity, space, key, region, machine, {
  success: function() {
  },
  failure: function(reason) {
  }
...
### Template
```js
connection.RegionalFinderFindbind(identity, space, key, region, machine, {
  success: function(response) {
    // response.id
    // response.locationType
    // response.archive
...
## Method: RegionalFinderDeleteMark
...
### Template
```js
connection.RegionalFinderDeleteMark(identity, space, key, region, machine, {
  success: function() {
  },
  failure: function(reason) {
  }
...
## Method: RegionalFinderDeleteCommit
Delete the document from the finder
...
### Template
```js
connection.RegionalFinderDeleteCommit(identity, space, key, region, machine, {
  success: function() {
  },
  failure: function(reason) {
  }
...
| region | yes | String | The region to bind too |
| machine | yes | String | The machine to bind too |
| archive | yes | String | The archive key to bind too |


### Template
```js
connection.RegionalFinderBackUp(identity, space, key, region, machine, archive, {
  success: function() {
  },
  failure: function(reason) {
  }
...
### Template
```js
connection.RegionalFinderList(identity, region, machine, {
  next: function(payload) {
    // payload.space
    // payload.key
  },
...
## Method: RegionalFinderDeletionList
...
### Template
```js
connection.RegionalFinderDeletionList(identity, region, machine, {
  next: function(payload) {
    // payload.space
    // payload.key
  },
