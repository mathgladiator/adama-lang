...
public class FileSystemDataServiceTests {
  ...
  public void testFlow() throws Exception {
    d("test_flow", (f) -> {
      ...
      f.create(new Key("s", "0"), id);
      Key key = new Key("s", id.get() + "");
      ...
      f.patch(key, new DataService.RemoteDocumentUpdate(1, NtClient.NO_ONE, "{}", "{\"x\":123}", "{\"x\":null}", false, 0), written);
      f.patch(key, new DataService.RemoteDocumentUpdate(1, NtClient.NO_ONE, "{}", "{\"x\":42}", "{\"x\":123}", false, 0), written);
      f.get(key, fetch);
  ...
  public void coverage() throws Exception {
    d("crash", (f) -> {
      Key key = new Key("s", "0");
      try {
        f.compute(key, DataService.ComputeMethod.Rewind, 1, null);
        Assert.fail();
      } catch (UnsupportedOperationException uoe) {
      }
      try {
        f.compute(key, DataService.ComputeMethod.Unsend, 1, null);
        Assert.fail();
      } catch (UnsupportedOperationException uoe) {
      }
    });
