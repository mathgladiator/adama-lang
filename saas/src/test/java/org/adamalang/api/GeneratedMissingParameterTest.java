...
public class GeneratedMissingParameterTest {
  ...
  public void missing() throws Exception {
    try (TestFrontEnd fe = new TestFrontEnd()) {
      ...
      Assert.assertEquals("ERROR:461826", c56.next());
      ...
      Assert.assertEquals("ERROR:454659", c66.next());
      node.put("channel", "xzya");
      Iterator<String> c67 = fe.execute(node.toString());
      Assert.assertEquals("ERROR:425987", c67.next());
      ...
      Assert.assertEquals("ERROR:405505", c68.next());
      node.put("connection", 100L);
      ...
      Assert.assertEquals("ERROR:405505", c71.next());
      node.put("connection", 100L);
      //ConnectionAttach
      node = Json.newJsonObject();
      node.put("id", 71);
      ...
      Assert.assertEquals("ERROR:470028", c84.next());
      node.put("filename", "xzya");
      Iterator<String> c85 = fe.execute(node.toString());
      Assert.assertEquals("ERROR:455691", c85.next());
      node.put("content-type", "xzya");
      //AttachmentAppend
      node = Json.newJsonObject();
      node.put("id", 85);
      ...
      Assert.assertEquals("ERROR:409609", c89.next());
      node.put("upload", 100L);
    }
  }
}
