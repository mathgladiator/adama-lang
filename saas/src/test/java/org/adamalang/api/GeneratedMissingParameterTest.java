...
public class GeneratedMissingParameterTest {
  ...
  public void missing() throws Exception {
    try (TestFrontEnd fe = new TestFrontEnd()) {
      ...
      Assert.assertEquals("ERROR:405505", c87.next());
      ...
      Assert.assertEquals("ERROR:454659", c91.next());
      node.put("channel", "xzya");
      ...
      Assert.assertEquals("ERROR:476156", c95.next());
      ...
      Assert.assertEquals("ERROR:477179", c98.next());
      node.put("size", 100L);
      Iterator<String> c99 = fe.execute(node.toString());
      Assert.assertEquals("ERROR:445437", c99.next());
      node.put("digest-md5", "xzya");
      Iterator<String> c100 = fe.execute(node.toString());
      Assert.assertEquals("ERROR:406525", c100.next());
      ...
      Assert.assertEquals("ERROR:409609", c112.next());
      node.put("upload", 100L);
      //SuperCheckIn
      node = Json.newJsonObject();
      node.put("id", 112);
      ...
      Assert.assertEquals("ERROR:439292", c115.next());
      ...
      Assert.assertEquals("ERROR:488444", c117.next());
      node.put("domain", "xzya");
      Iterator<String> c118 = fe.execute(node.toString());
      Assert.assertEquals("ERROR:439292", c118.next());
      node.put("timestamp", 100L);
    }
  }
}
