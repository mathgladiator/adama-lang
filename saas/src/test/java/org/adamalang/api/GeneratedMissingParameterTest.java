...
public class GeneratedMissingParameterTest {
  ...
  public void missing() throws Exception {
    try (TestFrontEnd fe = new TestFrontEnd()) {
      ...
      Assert.assertEquals("ERROR:473103", c35.next());
      node.put("email", "x@x.com");
      Iterator<String> c36 = fe.execute(node.toString());
      Assert.assertEquals("ERROR:456716", c36.next());
      node.put("role", "xzya");
      //SpaceReflect
      node = Json.newJsonObject();
      node.put("id", 36);
      ...
      Assert.assertEquals("ERROR:466947", c39.next());
      node.put("key", "xzya");
      //SpaceList
      node = Json.newJsonObject();
      node.put("id", 39);
      ...
      Assert.assertEquals("ERROR:466947", c43.next());
      node.put("key", "xzya");
      Iterator<String> c44 = fe.execute(node.toString());
      Assert.assertEquals("ERROR:461826", c44.next());
      ...
      Assert.assertEquals("ERROR:454659", c51.next());
