...
public class GeneratedMissingParameterTest {
  ...
  public void missing() throws Exception {
    try (TestFrontEnd fe = new TestFrontEnd()) {
      ...
      Assert.assertEquals("ERROR:470028", c122.next());
      node.put("filename", "xzya");
      Iterator<String> c123 = fe.execute(node.toString());
      Assert.assertEquals("ERROR:455691", c123.next());
      node.put("content-type", "xzya");
      //AttachmentAppend
      node = Json.newJsonObject();
      node.put("id", 123);
      ...
      Assert.assertEquals("ERROR:462859", c125.next());
      node.put("chunk-md5", "xzya");
      Iterator<String> c126 = fe.execute(node.toString());
      Assert.assertEquals("ERROR:409608", c126.next());
      ...
      Assert.assertEquals("ERROR:409609", c127.next());
      node.put("upload", 100L);
      //SuperCheckIn
      node = Json.newJsonObject();
      node.put("id", 127);
      ...
      Assert.assertEquals("ERROR:439292", c133.next());
      node.put("timestamp", 100L);
      ...
      Assert.assertEquals("ERROR:488444", c135.next());
      node.put("domain", "xzya");
      //RegionalEmitMetrics
      node = Json.newJsonObject();
      node.put("id", 135);
      ...
      Assert.assertEquals("ERROR:425213", c139.next());
