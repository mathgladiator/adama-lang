...
public class GeneratedMissingParameterTest {
  ...
  public void missing() throws Exception {
    try (TestFrontEnd fe = new TestFrontEnd()) {
      ...
      Assert.assertEquals("ERROR:402428", c34.next());
      ...
      Assert.assertEquals("ERROR:473103", c41.next());
      node.put("email", "x@x.com");
      Iterator<String> c42 = fe.execute(node.toString());
      Assert.assertEquals("ERROR:456716", c42.next());
      node.put("role", "xzya");
      //SpaceReflect
      node = Json.newJsonObject();
      node.put("id", 42);
      ...
      Assert.assertEquals("ERROR:466947", c45.next());
      node.put("key", "xzya");
      //SpaceList
      node = Json.newJsonObject();
      node.put("id", 45);
      ...
      Assert.assertEquals("ERROR:488444", c48.next());
      ...
      Assert.assertEquals("ERROR:454659", c70.next());
      node.put("channel", "xzya");
      Iterator<String> c71 = fe.execute(node.toString());
      Assert.assertEquals("ERROR:425987", c71.next());
      node.put("message", Json.newJsonObject());
      //ConnectionCanAttach
      node = Json.newJsonObject();
      node.put("id", 71);
      ...
      Assert.assertEquals("ERROR:405505", c72.next());
      node.put("connection", 100L);
      //ConnectionAttach
      node = Json.newJsonObject();
      node.put("id", 72);
      ...
      Assert.assertEquals("ERROR:470028", c85.next());
      node.put("filename", "xzya");
      Iterator<String> c86 = fe.execute(node.toString());
      Assert.assertEquals("ERROR:455691", c86.next());
      node.put("content-type", "xzya");
      //AttachmentAppend
      node = Json.newJsonObject();
      node.put("id", 86);
      ...
      Assert.assertEquals("ERROR:462859", c88.next());
      node.put("chunk-md5", "xzya");
      Iterator<String> c89 = fe.execute(node.toString());
      Assert.assertEquals("ERROR:409608", c89.next());
