...
public class DomainGetRequest {
  ...
  public DomainGetRequest(final String identity, final AuthenticatedUser who, final String domain, final DomainWithPolicy resolvedDomain) {
  ...
  public static void resolve(Session session, GlobalConnectionNexus nexus, JsonRequest request, Callback<DomainGetRequest> callback) {
    try {
      final BulkLatch<DomainGetRequest> _latch = new BulkLatch<>(nexus.executor, 2, callback);
      ...
      _latch.with(() -> new DomainGetRequest(identity, who.get(), domain, resolvedDomain.get()));
