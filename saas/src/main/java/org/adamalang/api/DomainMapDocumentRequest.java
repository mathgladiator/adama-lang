...
public class DomainMapDocumentRequest {
  ...
  public DomainMapDocumentRequest(final String identity, final AuthenticatedUser who, final String domain, final String space, final SpacePolicy policy, final String key, final String certificate) {
  ...
  public static void resolve(Session session, ConnectionNexus nexus, JsonRequest request, Callback<DomainMapDocumentRequest> callback) {
    try {
      final BulkLatch<DomainMapDocumentRequest> _latch = new BulkLatch<>(nexus.executor, 2, callback);
      ...
      _latch.with(() -> new DomainMapDocumentRequest(identity, who.get(), domain, space, policy.get(), key, certificate));
