...
public class ConnectionCreateRequest {
  ...
  public ConnectionCreateRequest(final String identity, final AuthenticatedUser who, final String space, final String key, final ObjectNode viewerState) {
  ...
  public static void resolve(Session session, RegionConnectionNexus nexus, JsonRequest request, Callback<ConnectionCreateRequest> callback) {
    try {
      final BulkLatch<ConnectionCreateRequest> _latch = new BulkLatch<>(nexus.executor, 1, callback);
      ...
      _latch.with(() -> new ConnectionCreateRequest(identity, who.get(), space, key, viewerState));
