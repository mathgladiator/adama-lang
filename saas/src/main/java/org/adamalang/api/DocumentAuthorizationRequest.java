...
public class DocumentAuthorizationRequest {
  ...
  public DocumentAuthorizationRequest(final String space, final SpacePolicy policy, final String key, final JsonNode message) {
    this.space = space;
    this.policy = policy;
    this.key = key;
  ...
  public static void resolve(Session session, RegionConnectionNexus nexus, JsonRequest request, Callback<DocumentAuthorizationRequest> callback) {
    try {
      final BulkLatch<DocumentAuthorizationRequest> _latch = new BulkLatch<>(nexus.executor, 1, callback);
      ...
      _latch.with(() -> new DocumentAuthorizationRequest(space, policy.get(), key, message));
      nexus.spaceService.execute(session, space, policy);
    } catch (ErrorCodeException ece) {
      nexus.executor.execute(new NamedRunnable("documentauthorization-error") {
        @Override
