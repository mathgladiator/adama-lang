...
public class AttachmentStartRequest {
  ...
  public AttachmentStartRequest(final String identity, final AuthenticatedUser who, final String space, final String key, final String filename, final String contentType) {
  ...
  public static void resolve(Session session, RegionConnectionNexus nexus, JsonRequest request, Callback<AttachmentStartRequest> callback) {
    try {
      final BulkLatch<AttachmentStartRequest> _latch = new BulkLatch<>(nexus.executor, 1, callback);
      ...
      _latch.with(() -> new AttachmentStartRequest(identity, who.get(), space, key, filename, contentType));
