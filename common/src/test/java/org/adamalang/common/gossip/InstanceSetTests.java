...
public class InstanceSetTests extends CommonTest {
  ...
  public void empty() {
    ...
    ArrayList<GossipProtocol.Endpoint> endpoints = set.toEndpoints();
    Assert.assertEquals(0, endpoints.size());
    ArrayList<GossipProtocol.Endpoint> missing = set.missing(set);
  ...
  public void singleton() {
    ...
    Assert.assertEquals("id-a", endpoints.get(0).id);
    ArrayList<GossipProtocol.Endpoint> missing1 = set.missing(set);
    Assert.assertEquals(0, missing1.size());
    ArrayList<GossipProtocol.Endpoint> missing2 = set.missing(new InstanceSet(new TreeSet<>(), 0));
    Assert.assertEquals(1, missing2.size());
    Assert.assertEquals("id-a", missing2.get(0).id);
  ...
  private void assertFour(TreeSet<Instance> instances) {
    ...
    Assert.assertEquals("id-a", endpoints.get(0).id);
    Assert.assertEquals("id-b", endpoints.get(1).id);
    Assert.assertEquals("id-c", endpoints.get(2).id);
    Assert.assertEquals("id-d", endpoints.get(3).id);
    ArrayList<GossipProtocol.Endpoint> missing1 = set.missing(set);
    Assert.assertEquals(0, missing1.size());
    ArrayList<GossipProtocol.Endpoint> missing2 = set.missing(new InstanceSet(new TreeSet<>(), 0));
    Assert.assertEquals(4, missing2.size());
    Assert.assertEquals("id-a", missing2.get(0).id);
    Assert.assertEquals("id-b", missing2.get(1).id);
    Assert.assertEquals("id-c", missing2.get(2).id);
    Assert.assertEquals("id-d", missing2.get(3).id);
    TreeSet<Instance> subset1 = new TreeSet<>();
    subset1.add(A());
    subset1.add(C());
    ArrayList<GossipProtocol.Endpoint> missing3 = set.missing(new InstanceSet(new TreeSet<>(subset1), 0));
    Assert.assertEquals(2, missing3.size());
    Assert.assertEquals("id-b", missing3.get(0).id);
    Assert.assertEquals("id-d", missing3.get(1).id);
