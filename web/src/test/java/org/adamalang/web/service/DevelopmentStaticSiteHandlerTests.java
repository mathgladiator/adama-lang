...
public class DevelopmentStaticSiteHandlerTests {

  @Test
  public void coverageOddity() throws Exception {
    Nexus nexus = NexusTests.mockNexus(NexusTests.Scenario.Dev);
    MockHttpCallback callback = new MockHttpCallback();
    nexus.passthroughHandler.handle(null, null, callback);
    callback.assertErrorCode(500);
  }
  ...
  public void special_404_not_found() throws Exception {
    FullHttpRequest request = new DefaultFullHttpRequest(HttpVersion.HTTP_1_1, HttpMethod.GET, "/nope");
    Nexus nexus = NexusTests.mockNexus(NexusTests.Scenario.DevScope);
    MockHttpCallback callback = new MockHttpCallback();
    nexus.passthroughHandler.handle(request, new QueryStringDecoder(request.uri()), callback);
    callback.awaitDone();
