...
public class DurableListStoreTests {
  ...
  public void flow() throws Exception {
    ...
    try {
      ...
      }
      {
        ...
        store.read(new Key("s", "42"), 42, stream);
      ...
      Assert.assertTrue(store.trim(new Key("s", "1"), 1, 90, event));
      Assert.assertFalse(store.trim(new Key("s", "1"),1, -1, event));
      {
        MockByteArrayStream stream = new MockByteArrayStream();
        store.read(new Key("s", "1"), 1, stream);
      ...
      store.delete(new Key("s", "1"),1, event);
      store.flush(false);
      Assert.assertEquals(105, count.get()); // 1 PUT, 100 PUT, 1 DELETE, 1 TRIM, 1 DELETE
      for (int k = 0; k < 1000; k++) {
        store.append(new Key("s", "" + k), k, ("K:" + k).getBytes(StandardCharsets.UTF_8), k, 1024, event);
      }
      for (int k = 0; k < 1000; k++) {
        Assert.assertTrue(store.delete(new Key("s", "" + k), k, event));
      }
      store.flush(true);
      Assert.assertFalse(store.delete(new Key("s", "102312412"), 102312412L, event));
      Assert.assertEquals(2105, count.get()); // + 1000 PUT, 1000 DELETE
      Assert.assertEquals(65536, store.available());
      int fill = 0;
      while (store.append(new Key("s", "1"), 1, ("01234567890123456789012345678901234567890123456789Junk@" + fill).getBytes(StandardCharsets.UTF_8), 0, 0, event) != null) {
        fill++;
      }
      Assert.assertFalse(store.append(new Key("s", "100"), 100, "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX".getBytes(StandardCharsets.UTF_8), 0, 1024, event) != null);
      Assert.assertEquals(3234, count.get());
      store.delete(new Key("s", "1"), 1, event);
      store.flush(false);
      Assert.assertEquals(3235, count.get());
      Assert.assertTrue(store.append(new Key("s", "100"), 100, "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX".getBytes(StandardCharsets.UTF_8), 1, 128, event) != null);
      store.flush(false);
  ...
  public void batch() throws Exception {
    ...
    try {
      ...
      {
        MockByteArrayStream stream = new MockByteArrayStream();
        store.read(new Key("s", "42"), 42, wrap(stream));
      ...
      Assert.assertTrue(store.trim(new Key("s", "42"), 42, 1, () -> {}));
      Assert.assertEquals(10534, store.available());
      Assert.assertTrue(store.trim(new Key("s", "100"), 100, 10, () -> {}));
      Assert.assertEquals(64922, store.available());
      Assert.assertTrue(store.trim(new Key("s", "100"), 100, 1, () -> {}));
  ...
  public void load() throws Exception {
    ...
    try {
      // public DurableListStore(File storeFile, File walRoot, long size, int initialWriteBufferSize, int flushCutOffBytes, long maxLogSize) throws IOException {
      {
        ...
        AtomicInteger count = new AtomicInteger(0);
        Runnable event = () -> { count.incrementAndGet(); };
        store.append(new Key("s", "42"), 42, "XYZ".getBytes(StandardCharsets.UTF_8), 10, 1024, event);
        for (int k = 0 ; k < 100; k++) {
          store.append(new Key("s", "1"), 1, ("K:" + k).getBytes(StandardCharsets.UTF_8), 10, 1024, event);
          store.append(new Key("s", "2"), 2, ("K:" + k).getBytes(StandardCharsets.UTF_8), 10, 1024, event);
          store.append(new Key("s", "3"), 3, ("K:" + k).getBytes(StandardCharsets.UTF_8), 10, 1024, event);
        }
