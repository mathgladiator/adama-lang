...
public class DurableListStoreTests {
  ...
  public void flow() throws Exception {
    ...
    try {
      ...
      Assert.assertFalse(store.exists(new Key("s", "42")));
      store.append(new Key("s", "42"), "XYZ".getBytes(StandardCharsets.UTF_8), 0, 42L, event);
      Assert.assertTrue(store.exists(new Key("s", "42")));
      for (int k = 0 ; k < 100; k++) {
        store.append(new Key("s", "1"), ("K:" + k).getBytes(StandardCharsets.UTF_8), k + 1, 100L, event);
      }
      {
        MockByteArrayStream stream = new MockByteArrayStream();
        store.read(new Key("s", "42"), stream);
        stream.assertIs("[0=XYZ/0:42]FINISHED");
      }
      store.delete(new Key("s", "42"), event);
      Assert.assertFalse(store.exists(new Key("s", "42")));
      {
        MockByteArrayStream stream = new MockByteArrayStream();
        store.read(new Key("s", "1"), stream);
      ...
      Assert.assertTrue(store.trim(new Key("s", "1"), 90, event));
      Assert.assertFalse(store.trim(new Key("s", "1"), -1, event));
      {
        MockByteArrayStream stream = new MockByteArrayStream();
        store.read(new Key("s", "1"), stream);
      ...
      store.delete(new Key("s", "1"),event);
      store.flush(false);
      Assert.assertEquals(105, count.get()); // 1 PUT, 100 PUT, 1 DELETE, 1 TRIM, 1 DELETE
      for (int k = 0; k < 1000; k++) {
        store.append(new Key("s", "" + k), ("K:" + k).getBytes(StandardCharsets.UTF_8), k, 1024, event);
      }
      for (int k = 0; k < 1000; k++) {
        Assert.assertTrue(store.delete(new Key("s", "" + k), event));
      }
      store.flush(true);
      Assert.assertFalse(store.delete(new Key("s", "102312412"), event));
      Assert.assertEquals(2105, count.get()); // + 1000 PUT, 1000 DELETE
      Assert.assertEquals(65536, store.available());
      int fill = 0;
      while (store.append(new Key("s", "1"), ("01234567890123456789012345678901234567890123456789Junk@" + fill).getBytes(StandardCharsets.UTF_8), 0, 0, event) != null) {
        fill++;
      }
      Assert.assertFalse(store.append(new Key("s", "100"), "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX".getBytes(StandardCharsets.UTF_8), 0, 1024, event) != null);
      Assert.assertEquals(3234, count.get());
      store.delete(new Key("s", "1"), event);
      store.flush(false);
      Assert.assertEquals(3235, count.get());
      Assert.assertTrue(store.append(new Key("s", "100"), "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX".getBytes(StandardCharsets.UTF_8), 1, 128, event) != null);
      store.flush(false);
  ...
  public void batch() throws Exception {
    ...
    try {
      ...
      Assert.assertEquals(65536, store.available());
      store.append(new Key("s", "42"), batch1, 10, 1024, event);
      Assert.assertEquals(60093, store.available());
      store.flush(false);
      {
        MockByteArrayStream stream = new MockByteArrayStream();
        store.read(new Key("s", "42"), wrap(stream));
      ...
      }
      {
        ...
        Assert.assertEquals(1025, (int) store.append(new Key("s", "100"), batch2, 10, 1024, event));
        {
          MockByteArrayStream stream = new MockByteArrayStream();
          store.read(new Key("s", "100"), wrap(stream));
      ...
      Assert.assertEquals(3806, store.available());
      {
        ...
        Assert.assertNull(store.append(new Key("s", "100"), batch3, 10, 1024, event));
      }
      Assert.assertEquals(3806, store.available());
      Assert.assertTrue(store.trim(new Key("s", "100"), 1000, () -> {}));
      Assert.assertEquals(5145, store.available());
      Assert.assertTrue(store.trim(new Key("s", "42"), 1, () -> {}));
      Assert.assertEquals(10534, store.available());
      Assert.assertTrue(store.trim(new Key("s", "100"), 10, () -> {}));
      Assert.assertEquals(64922, store.available());
      Assert.assertTrue(store.trim(new Key("s", "100"), 1, () -> {}));
    ...
    try {
      {
        ...
        AtomicInteger count = new AtomicInteger(0);
        Runnable event = () -> { count.incrementAndGet(); };
        store.append(new Key("s", "42"), "XYZ".getBytes(StandardCharsets.UTF_8), 10, 1024, event);
        for (int k = 0 ; k < 100; k++) {
          store.append(new Key("s", "1"), ("K:" + k).getBytes(StandardCharsets.UTF_8), 10, 1024, event);
          store.append(new Key("s", "2"), ("K:" + k).getBytes(StandardCharsets.UTF_8), 10, 1024, event);
          store.append(new Key("s", "3"), ("K:" + k).getBytes(StandardCharsets.UTF_8), 10, 1024, event);
        }

        store.trim(new Key("s", "2"), 5, event);
        Assert.assertTrue(store.exists(new Key("s", "3")));
        store.delete(new Key("s", "3"), event);
        Assert.assertFalse(store.exists(new Key("s", "3")));
        store.flush(false);
        store.shutdown();
      }
      {
